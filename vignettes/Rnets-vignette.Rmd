...
The estimated structure of the network is conditional on the selected L~1~. `L1Selection()` discussed below uses a resampling and stability method assist in selecting an L~1~ penalty objectively.
...
The results of ```Rnet()```, in addition to the input arguments are stored in the S4 classes `rnetBasic`, `rnetSubset`, and `rnetStrata`. The specific output class is determined by the optional `subset` arugment as discussed above.  
...
### Assigning network metadata

### Network visualization

Two primary methods are used to visualize the Rnets: individual networks can be visualized with the generic method ```plot()``` and heatmaps can be used visualize the edges in multiple graphs using `Rnet_Heatmap()`. 
