...
####rnetSubset
`rnetSubset` objects inherit from `rnetBasic`, and gain the `@subset` slot, which contains the expression used to define the subset.

####rnetStrata
`rnetStrata` does not inherit from either of the two other classes. 
...
This method is used to simultaneously assign multiple vertex attributes to an `igraph` networks, including those stored in `rnetbasic@R`. The vertex attributes are stored in data.frame that can be viewed easily. An example frame of vertex attributes is included in the package
```{r}
Rnets::V_ATTRS[1:10,]
```
...
We could use a series of four `V(rnetbasic.obj@R)$attr` or `set_vertex_attr(...)` calls (one for each additional attribute to add), but it is more efficient to use `Assign_Vmetadata`. We will the vertex attribute `name`( `V(rnetbasic.obj@R)$name`)  to entries `V_ATTRS$Code` to assign the data.

### Network visualization

Two primary methods are used to visualize the R-nets: individual networks can be visualized with the generic method ```plot()``` and heatmaps can be used visualize the edges in multiple graphs using `Rnet_Heatmap()`. 
